...
module "Test_ALB" {
  ...
  project_name               = var.project_name
  open_cidr                  = var.open_cidr
  alb_cidr                   = module.Test_VPC.cidr_block
...
module "RDS" {
  ...
  alb_sg                       = module.Test_ALB.alb_sg_id
  dt_vpc                       = module.Test_VPC.vpc_id
  rds_sg_ingress               = var.rds_sg_ingress
  rds_sg_egress                = var.rds_sg_egress
  project_name                 = var.project_name
  rds_sg_cidr                  = module.Test_VPC.cidr_block
}
...
module "Test_ECS" {
  ...
  ecs_vpc_id                     = module.Test_VPC.vpc_id
  ecr_repo_url                   = module.Test_ecr.ecr_repo_url
  dt_ecs_task_family             = var.dt_ecs_task_family
  dt_ecs_task_name               = var.dt_ecs_task_name
  container_port                 = var.container_port
  ecs_task_execution_role        = module.Test_role.ecs_task_execution_role
  aws_iam_role_policy_attachment = module.Test_role.ecs_iam_role_policy_attachment
  aws_iam_policy_document        = module.Test_role.iam_policy_document
  lb_target_group                = module.Test_ALB.target-group
  dt_ecs_service_name            = var.dt_ecs_service_name
  ecs-lb                         = module.Test_ALB.alb_sg_id
  ecs_priv_subs                  = module.Test_VPC.private_subnets
  ecs_service_sg_ingress         = var.ecs_service_sg_ingress
  ecs_service_sg_egress          = var.ecs_service_sg_egress
  ecs_cidr_block                 = module.Test_VPC.cidr_block
  project_name                   = var.project_name
  ...
  log_group_name                 = module.Test_cloudwatch.ecs_log_group_name
...
module "Test_route53" {
  ...
  route53_lb_dns_name = module.Test_ALB.load_balancer_dns_name
  domain              = var.domain
